...
class Conv3dLayer(torch.nn.Conv3d):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if self.padding_mode != 'zeros':
            return F.conv3d(F.pad(input, self._reversed_padding_repeated_twice, mode=self.padding_mode),
                            self.weight.to(input.dtype), b, self.stride, _triple(0),
                            self.dilation, self.groups)
        return F.conv3d(input, self.weight.to(input.dtype), b, self.stride,
...
class AddCoordsTh(nn.Module):
    ...
    def _get_coords(self, dtype=torch.float16):
        ...
        #xx_channel = xx_channel.repeat(1, 1, 1, 1)
        #yy_channel = yy_channel.repeat(1, 1, 1, 1)
    ...
    def forward(self, input_tensor):
        ...
        coords = self.coords.repeat(batch_size_tensor, 1, 1, 1).to(input_tensor.dtype)

        ret = torch.cat([input_tensor, coords], dim=1)
        return ret
...
class LambdaLayer(torch.nn.Module):
    def __init__(
        ...
        self.to_q = Conv2dLayer(dim, dim_k * heads, 1, bias=False, conv_clamp=conv_clamp)
        self.to_k = Conv2dLayer(dim, dim_k * dim_u, 1, bias=False, conv_clamp=conv_clamp)
        self.to_v = Conv2dLayer(dim, dim_v * dim_u, 1, bias=False, conv_clamp=conv_clamp)

        self.norm_q = nn.InstanceNorm2d(dim_k * heads, affine=False)
        self.norm_v = nn.InstanceNorm2d(dim_v * dim_u, affine=False)

        self.local_contexts = exists(r)
        if exists(r):
            assert (r % 2) == 1, 'Receptive kernel size should be odd'
            self.pos_conv = Conv3dLayer(dim_u, dim_k, (1, r, r), padding=(0, r // 2, r // 2))
        else:
    ...
    def forward(self, x, gain=1):
        ...
        q = self.to_q(x, gain=gain)
        k = self.to_k(x, gain=gain)
        v = self.to_v(x, gain=gain)
...
class SynthesisLayer(torch.nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1, rand_gen=None):
        assert noise_mode in ['random', 'const', 'none', 'generator']
        ...
        if self.use_noise and noise_mode == 'generator':
            noise = torch.randn([x.shape[0], 1, self.resolution, self.resolution],
                                device=x.device, generator=rand_gen) * self.noise_strength
...
class SynthesisBlock(torch.nn.Module):
    def __init__(self,
        ...
        conv_1_in_channels = out_channels
        self.use_coordinates = use_coordinates

        if self.use_coordinates and (in_channels != 0):
            self.coords = AddCoordsTh(resolution, resolution, True)
            conv_1_in_channels = 3 + conv_1_in_channels

        self.conv1 = SynthesisLayer(conv_1_in_channels, out_channels, w_dim=w_dim, resolution=resolution,
        ...
        self.use_lambda_layer = use_lambda_layer
        if self.use_lambda_layer:
            self.lambdaLayer = LambdaLayer(
                dim=out_channels,
                dim_out=out_channels,
                r=17,  # the receptive field for relative positional encoding (23 x 23)
                dim_k=8,
                heads=4,
                dim_u=1,
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
        ...
        if self.in_channels == 0:
            ...
            if self.use_coordinates and (self.in_channels != 0):
                x = self.coords(x)
    ...
    def __init__(self,
        ...
        for res in self.block_resolutions:
            ...
            block = SynthesisBlock(in_channels, out_channels, w_dim=w_dim, resolution=res,
                                   ...
                                   use_lambda_layer=use_lambda and (res == lambda_res),
...
class DiscriminatorBlock(torch.nn.Module):
    def __init__(self,
        ...
        conv_1_in_channels = tmp_channels
        if self.use_coordinates:
            conv_1_in_channels += 3
            self.coords = AddCoordsTh(resolution, resolution, True)

        self.conv1 = Conv2dLayer(conv_1_in_channels, out_channels, kernel_size=3, activation=activation, down=2,
